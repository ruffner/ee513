...
batar = .75; %  Beta values for PHAT processing
...
% create plot
std=fliplr([0.77,0.31,0.26]); %std deviation
var=fliplr([0.6,0.1,0.07]); % variance
avg=fliplr([6.32,7.17,7.01]); % averages
spa=fliplr([0.75,0.5,0.2]); % spacings
...
hold on
plot(spa,avg,'r-*')
errorbar(spa,avg,std)
errorbar(spa,avg,var)
title('Steered Coherent Response Power vs Microphone Array Spacing')
...
% moved the mic placement up here in the algorithm for accurate mic counts
mints=[0.75 0.5 0.2];
for mint=mints
    mposperim = [regmicsline([v(:,1) v(:,2)], mint) regmicsline([v(:,1) v(:,4)], mint)];
    micnum = size(mposperim,2);%micnum = 8;  %  Number of mics in array to be tested
    ...
    % matt ruffner 2/28/17 - moved earlier so micnum is accurate
    % 
    %mposperim = regmicsperim(v(:,[1,3]),spm,stp);
    %mposperim = [regmicsline([v(:,1) v(:,2)], .75) regmicsline([v(:,1) v(:,4)], .75)];
    ...
    % matt ruffner 2/28/17 place target in constatnt center to test mic spacing
    ...
    sigout = whiten(sigoutpera(sst:sed,:).*wintap, batar); % matt ruffner
